...
try{
  ...
  const report=await page.evaluate(()=>{
    ...
    const pigeon=afterRoam.pigeons[0];
    TV.player.position.set(pigeon.x,TV.terrainHeight(pigeon.x,pigeon.z),pigeon.z);
    W.advance(.1);
    ...
    const butterfly=landed.butterflies[0];
    TV.player.position.set(butterfly.x,TV.terrainHeight(butterfly.x,butterfly.z),butterfly.z);
    W.advance(.1);
  ...
  if(report.afterPlayerScatter.playerScatters<=report.afterRoam.playerScatters||report.afterPlayerScatter.pigeons[0].scatterCount<=report.afterRoam.pigeons[0].scatterCount)throw new Error(`Pigeon did not react to nearby player ${JSON.stringify({before:report.afterRoam,after:report.afterPlayerScatter})}`);
  ...
  if(report.afterButterflyDodge.butterflyDodges<=report.landed.butterflyDodges||report.afterButterflyDodge.butterflies[0].dodgeCount<=report.landed.butterflies[0].dodgeCount)throw new Error(`Butterfly did not evade nearby player ${JSON.stringify({before:report.landed,after:report.afterButterflyDodge})}`);
