...
try{
 ...
 const report=await page.evaluate(()=>{
  ...
  const turtle=calm.turtles[0];TV.player.position.set(turtle.x,turtle.y,turtle.z);S.advance(.1);const afterTurtle=S.getState();
  ...
  return {flags:{active:S.active,turtles:S.baskingTurtles,dive:S.physicalDiveAndReturn,minnows:S.reactiveMinnowSchool,isolated:S.isolatedFromFishing,budget:S.lowPopulationBudget},lake:L.lake,calm,afterTurtle,turtleCycle,afterMinnow,settled,root:Boolean(root),children:root?.children.length||0};
 });
 ...
 if(report.afterTurtle.turtleDives<=report.calm.turtleDives||report.afterTurtle.turtles[0].state!=='diving')throw new Error(`Turtle did not dive ${JSON.stringify({before:report.calm.turtles[0],after:report.afterTurtle.turtles[0]})}`);
 if(report.turtleCycle.turtles[0].state!=='basking')throw new Error(`Turtle did not complete swim/return cycle ${JSON.stringify(report.turtleCycle.turtles[0])}`);
 ...
 const l=report.lake;
 if(report.settled.turtles.some(t=>!Number.isFinite(t.x)||!Number.isFinite(t.y)||!Number.isFinite(t.z)||Math.abs(t.x-l.x)>l.rx*1.1||Math.abs(t.z-l.z)>l.rz*1.1))throw new Error(`Turtle left lake bounds ${JSON.stringify(report.settled.turtles)}`);
 if(report.settled.minnows.some(m=>!Number.isFinite(m.x)||!Number.isFinite(m.y)||!Number.isFinite(m.z)||m.y<0||Math.abs(m.x-l.x)>l.rx*1.1||Math.abs(m.z-l.z)>l.rz*1.1))throw new Error(`Minnow state invalid ${JSON.stringify(report.settled.minnows)}`);
