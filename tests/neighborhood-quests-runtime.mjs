...
try{
  ...
  const report=await page.evaluate(()=>{
    ...
    TV.enterInterior('clinic',{x:-75,z:-9});const clinicStart=find('Ask Rosa about clinic deliveries');if(!enabled(clinicStart))throw new Error('Clinic quest start unavailable');clinicStart.action();TV.exitInterior();
    let clinic=N.getState().clinic;if(clinic.stage!=='pickup')throw new Error(`Clinic did not enter pickup stage: ${JSON.stringify(clinic)}`);
    TV.enterInterior('cafe',{x:-15,z:28.1});const soup=find('Pick up recovery soup from Ari');if(!enabled(soup))throw new Error('Clinic soup pickup unavailable');soup.action();TV.exitInterior();
    clinic=N.getState().clinic;if(clinic.stage!=='return')throw new Error(`Clinic did not enter return stage: ${JSON.stringify(clinic)}`);
    TV.enterInterior('clinic',{x:-75,z:-9});const clinicReturn=find('Deliver recovery soup to Rosa');if(!enabled(clinicReturn))throw new Error('Clinic return unavailable');clinicReturn.action();TV.exitInterior();
    clinic=N.getState().clinic;

    TV.enterInterior('school',{x:72,z:-35});const schoolStart=find('Ask Ms. Maple about classroom supplies');if(!enabled(schoolStart))throw new Error('School quest start unavailable');schoolStart.action();TV.exitInterior();
    let school=N.getState().school;if(school.stage!=='pickup')throw new Error(`School did not enter pickup stage: ${JSON.stringify(school)}`);
    TV.enterInterior('generalStore',{x:26,z:-19.6});const art=find('Collect classroom art kit from Nina');if(!enabled(art))throw new Error('Art kit pickup unavailable');art.action();TV.exitInterior();
    school=N.getState().school;if(school.stage!=='return')throw new Error(`School did not enter return stage: ${JSON.stringify(school)}`);
    TV.enterInterior('school',{x:72,z:-35});const schoolReturn=find('Give art kit to Ms. Maple');if(!enabled(schoolReturn))throw new Error('School return unavailable');schoolReturn.action();TV.exitInterior();
