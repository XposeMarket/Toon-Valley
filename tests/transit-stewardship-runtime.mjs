...
try{
  ...
  if(!report.destinationTrips||!report.physicalDisembark||!report.dynamicStopBoards||!report.visibleBusPassengers||!report.destinationWalks||report.boardCount!==4||report.passengerSlotCount!==8||report.destinationCount!==4)throw new Error(`Transit rider-life improvements failed to initialize ${JSON.stringify(report)}`);
