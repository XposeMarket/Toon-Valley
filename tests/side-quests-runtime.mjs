...
try{
  ...
  await page.waitForFunction(()=>window.ToonValleySideQuests&&window.ToonValleyServices&&window.ToonValleyLife?.getState()?.player,null,{timeout:45000});
  await page.click('#play-button');await wait(160);

  const report=await page.evaluate(async()=>{
    const TV=window.ToonValley,Q=window.ToonValleySideQuests,S=window.ToonValleyServices,Life=window.ToonValleyLife;
    ...
    // Lost pet: physical petting starts an escort, the pet follows, and only the real owner door can finish it.
    const pet=byPrompt('Help Mochi get home');
    const petObject=pet.object,petBefore={x:petObject.position.x,z:petObject.position.z};
    pet.action();await sleep(1120);
    const petStart=S.getState();
    TV.player.position.set(petBefore.x+5,TV.terrainHeight(petBefore.x+5,petBefore.z),petBefore.z);
    const distanceBefore=Math.hypot(petObject.position.x-TV.player.position.x,petObject.position.z-TV.player.position.z);
    await sleep(950);
    const distanceAfter=Math.hypot(petObject.position.x-TV.player.position.x,petObject.position.z-TV.player.position.z),moved=Math.hypot(petObject.position.x-petBefore.x,petObject.position.z-petBefore.z);
    const home=S.petHomes.find(h=>h.name==='Mochi');
    TV.player.position.set(home.x,TV.terrainHeight(home.x,home.z),home.z);
    petObject.position.set(home.x+1,TV.terrainHeight(home.x+1,home.z),home.z+.5);
    const knock=byPrompt("Knock on Mrs. Juniper's door");
    if(!enabled(knock))throw new Error(`Lost pet real-house door did not activate: ${JSON.stringify(petStart)}`);
    knock.action();await sleep(120);
    const petDone=S.getState(),ownerVisible=knock.object.children.some(c=>c.visible&&c.userData?.arms);

    // Forage: four bushes fill a carried basket; cafe handoff is the completion.
    const berries=TV.interactables.filter(i=>i.prompt==='Gather wild berries').slice(0,4);for(const i of berries){if(enabled(i))i.action()}
    ...
    const litter=TV.interactables.filter(i=>i.prompt==='Pick up litter');for(const i of litter){if(enabled(i)){i.action();await sleep(620)}}
    const cleanupReady=Q.getState().cleanup;const bin=byPrompt('Drop cleanup bag in park bin');if(!enabled(bin))throw new Error('Cleanup return bin did not activate');bin.action();const cleanupDone=Q.getState().cleanup;

    // Bird survey: notebook checkout -> four observations -> library return.
    TV.enterInterior('library',{x:-26,z:-19.6});const checkout=byPrompt('Check out bird survey notebook');if(!enabled(checkout))throw new Error('Bird notebook checkout missing');checkout.action();TV.exitInterior();
    const birds=TV.interactables.filter(i=>/^Observe /.test(i.prompt||''));for(const i of birds){if(enabled(i)){i.action();await sleep(620)}}
    const birdReady=Q.getState().birds;TV.enterInterior('library',{x:-26,z:-19.6});const birdReturn=byPrompt('Return completed bird survey to Mabel');if(!enabled(birdReturn))throw new Error('Bird survey return missing');birdReturn.action();TV.exitInterior();const birdDone=Q.getState().birds;

    // Garden: borrow can -> water six individual beds -> return equipment.
    Q.startGarden();const gardenBeds=TV.interactables.filter(i=>/^Water raised garden bed /.test(i.prompt||''));for(const i of gardenBeds){if(enabled(i))i.action()}
    const gardenReady=Q.getState().garden;const canReturn=byPrompt('Return watering can');if(!enabled(canReturn))throw new Error('Watering-can return missing');canReturn.action();const gardenDone=Q.getState().garden;

    // Notice board: accept -> collect actual cargo from a worker -> deliver to the destination.
    ...
    Q.marketAction();const samples=TV.interactables.filter(i=>/^Taste market sample /.test(i.prompt||''));for(const i of samples){if(enabled(i))i.action()}
    const marketReady=Q.getState().market;Q.marketAction();const marketDone=Q.getState().market;

    const oldErrand=byPrompt('Complete community errand'),oldWater=TV.interactables.filter(i=>i.prompt==='Water community garden bed');
    return {
  ...
  if(report.counts.overhauled<6||report.audit.alreadySubstantive.length<9)throw new Error(`Quest audit incomplete ${JSON.stringify(report)}`);
