...
try{
  ...
  const report=await page.evaluate(()=>{
    ...
    let streamSeen=false,bottleMoved=false,stretchSeen=false;
    for(let i=0;i<900;i++){
      ...
      if(ps.hydrationSequences>=2&&ps.stretchSessions>=2&&streamSeen&&bottleMoved&&stretchSeen)break;
    }
    ...
    if(!target)return {start,afterRoutines,streamSeen,bottleMoved,stretchSeen,target:null,facingError:Infinity,afterYield:null,rootPresent:Boolean(TV.scene.getObjectByName('ambient-pedestrian-polish'))};
    ...
    return {start,afterRoutines,streamSeen,bottleMoved,stretchSeen,target,afterYield,facingError,stationNames,rootPresent:Boolean(TV.scene.getObjectByName('ambient-pedestrian-polish'))};
  });
  if(!report.rootPresent||!report.start.finitePositions)throw new Error(`Ambient polish world failed ${JSON.stringify(report)}`);
  if(report.start.hydrationStationCount!==4||report.start.stretchStationCount!==4)throw new Error(`Park station population failed ${JSON.stringify(report.start)}`);
  if(report.stationNames?.filter(name=>name.startsWith('sunshine-hydration-station-')).length!==4||report.stationNames?.filter(name=>name.startsWith('sunshine-stretch-station-')).length!==4)throw new Error(`Physical park station scene objects missing ${JSON.stringify(report.stationNames)}`);
  ...
  if(report.afterRoutines.stretchSessions<2||!report.afterRoutines.stationStretchSessions.some(n=>n>0)||!report.stretchSeen)throw new Error(`Physical stretch sessions failed ${JSON.stringify(report)}`);
  ...
  console.log('Sunshine Park refill/sip, stretch stations, and global pedestrian yield-facing passed runtime checks',report);
