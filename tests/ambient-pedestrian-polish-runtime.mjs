...
try{
  ...
  const report=await page.evaluate(()=>{
    ...
    let streamSeen=false,bottleMoved=false,stretchSeen=false,benchSeen=false,viewpointSeen=false,photoPhoneSeen=false,seatedOnPhysicalBench=false;
    for(let i=0;i<1200;i++){
      ...
      const lifeRoot=TV.scene.getObjectByName('ambient-pedestrian-life');
      for(const child of lifeRoot?.children||[]){
        ...
        if(child.userData.toonValleyBenchSeated&&child.userData.toonValleyRestBench){
          const bench=TV.scene.getObjectByName(child.userData.toonValleyRestBench);
          const body=child.getObjectByName('body');
          if(bench&&Math.hypot(child.position.x-bench.position.x,child.position.z-bench.position.z)<.82&&Math.abs((body?.rotation.x||0)+.12)<.03)seatedOnPhysicalBench=true;
        }
      }
      if(ps.hydrationSequences>=2&&ps.stretchSessions>=2&&ps.benchRestSessions>=2&&ps.viewpointPhotoSessions>=2&&ps.stretchPoseResets>0&&streamSeen&&bottleMoved&&stretchSeen&&benchSeen&&viewpointSeen&&photoPhoneSeen&&seatedOnPhysicalBench)break;
    }
    ...
    if(!target)return {start,afterRoutines,streamSeen,bottleMoved,stretchSeen,benchSeen,viewpointSeen,photoPhoneSeen,seatedOnPhysicalBench,target:null,facingError:Infinity,afterYield:null,rootPresent:Boolean(TV.scene.getObjectByName('ambient-pedestrian-polish'))};
    ...
    return {start,afterRoutines,streamSeen,bottleMoved,stretchSeen,benchSeen,viewpointSeen,photoPhoneSeen,seatedOnPhysicalBench,target,afterYield,facingError,stationNames,rootPresent:Boolean(TV.scene.getObjectByName('ambient-pedestrian-polish'))};
  });
  ...
  if(report.afterRoutines.benchRestSessions<2||!report.afterRoutines.stationBenchSessions.some(n=>n>0)||!report.benchSeen||!report.seatedOnPhysicalBench)throw new Error(`Physical bench-rest sessions failed ${JSON.stringify(report)}`);
  ...
  if(report.afterRoutines.benchRestSessions>40||report.afterRoutines.viewpointPhotoSessions>40)throw new Error(`Park activity sessions appear frame-counted ${JSON.stringify(report.afterRoutines)}`);
