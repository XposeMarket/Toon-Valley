...
assert.match(ui,/armResumeAfterModal/);
assert.match(ui,/requestAnimationFrame\(\(\)=>\{/);
assert.match(ui,/Life\.openPhone\(tab\)/);
assert.match(ui,/beginPopoverTransition\(\)/);
assert.match(ui,/keepsWebGLRenderingUnderModal:true/);
assert.match(ui,/freezesWebGLDrawsUnderModal:false/);
assert.match(ui,/replacesRendererRender:false/);
assert.match(ui,/preopensModalState:false/);
assert.match(ui,/pointerUnlockBeforeModalConstruction:true/);
assert.doesNotMatch(ui,/TV\.renderer\.render\s*=/);
assert.doesNotMatch(ui,/TV\.setModalOpen\(true\)/);
...
console.log('navigation, desktop controls, popover rendering, and fishing static checks passed');
