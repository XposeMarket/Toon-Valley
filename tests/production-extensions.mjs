...
try{
 ...
 if(!s.world.pondMoved||s.world.roadsAdded<4)throw new Error(`Production world polish missing ${JSON.stringify(s.world)}`);
 if(s.lake.lakes!==1||s.lake.boats!==1||s.lake.reeds<30||s.lake.lilyPads<8)throw new Error(`Production lake incomplete ${JSON.stringify(s.lake)}`);
 ...
 if(!s.mobile.active||!s.mobile.sprintPointerFix||!s.mobile.portraitLandscape)throw new Error(`Production mobile polish missing ${JSON.stringify(s.mobile)}`);
 if(s.interactables<70)throw new Error(`Production interaction count too low ${s.interactables}`);if(errors.length)throw new Error(errors.join('\n'));console.log('Production world extensions verified',s);
