...
if (!external) {
  server = spawn('python3', ['-m', 'http.server', '4176', '--bind', '127.0.0.1'], { stdio: ['ignore', 'pipe', 'pipe'] });
  await wait(900);
}
const base = (external || 'http://127.0.0.1:4176').replace(/\/$/, '');
...
try {
  ...
  await page.waitForFunction(() => window.ToonValleyPublicInteriors && window.ToonValleyTheater, null, { timeout: 45000 });
  ...
  if (!state.theaterEntrance || state.theaterSeats < 8) throw new Error(`Theater interactions missing: ${JSON.stringify(state)}`);
  if (errors.length) throw new Error(errors.join('\n'));
  console.log('public interiors and theater runtime checks passed', state);
} finally {
  await browser.close();
