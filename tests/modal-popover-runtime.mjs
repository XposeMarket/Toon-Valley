...
try{
  ...
  await page.evaluate(()=>{const TV=window.ToonValley;TV.state.started=true;document.getElementById('start-screen')?.classList.add('hidden');document.getElementById('hud')?.classList.remove('hidden');document.getElementById('pause-screen')?.classList.add('hidden')});

  await moveTo('home','Open decorating menu');await openCurrent('home decorating');await closeModal('home decorating');
  await page.evaluate(()=>document.getElementById('pause-screen')?.classList.add('hidden'));
