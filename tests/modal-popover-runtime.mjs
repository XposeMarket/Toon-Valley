...
async function requireReleasedForModal(label) {
  ...
  await page.waitForFunction(() => !document.pointerLockElement && window.ToonValley.state.modalOpen === true);
...
async function closeAndRequireGameplay(label) {
  await page.click('.life-close');
  await page.waitForFunction(() => !document.querySelector('.life-overlay') && window.ToonValley.state.modalOpen === false);
  await requireGamePointerLock(`${label} close`);
  const pauseHidden = await page.evaluate(() => document.getElementById('pause-screen').classList.contains('hidden'));
  if (!pauseHidden) throw new Error(`${label}: pause overlay appeared after modal close`);
}
...
try {
  ...
  const guard = await page.evaluate(() => window.ToonValleyPointerGuard);
  if (!guard.modalPointerRestore || !guard.nativeExitPointerLock) throw new Error(`Pointer guard capabilities missing ${JSON.stringify(guard)}`);
  ...
  await closeAndRequireGameplay(npcPrompt);

  // Prove that closing the interaction did not leave desktop gameplay invisibly paused.
  ...
  const replacement = await page.evaluate(() => ({ modalOpen: window.ToonValley.state.modalOpen, pointerLocked: Boolean(document.pointerLockElement) }));
  if (!replacement.modalOpen || replacement.pointerLocked) throw new Error(`ToonPhone modal replacement regression ${JSON.stringify(replacement)}`);
  await closeAndRequireGameplay('ToonPhone replacement');

  if (errors.length) throw new Error(errors.join('\n'));
  console.log(`Toon Valley modal/popover lifecycle passed: ${remoteURL || 'localhost'}`, { npcPrompt, before, after });
} finally {
  await browser.close();
  server?.kill('SIGTERM');
}
