...
try{
  ...
  await moveTo('home','Open decorating menu');await openWithE('home decorating');await closeAndResume('home decorating');
  await moveTo('furnitureStore','Browse furniture catalog');await openWithE('furniture catalog');await closeAndResume('furniture catalog');

  const before=await page.evaluate(()=>({x:window.ToonValley.player.position.x,z:window.ToonValley.player.position.z}));await page.keyboard.down('KeyW');await wait(450);await page.keyboard.up('KeyW');const after=await page.evaluate(()=>({x:window.ToonValley.player.position.x,z:window.ToonValley.player.position.z}));if(Math.hypot(after.x-before.x,after.z-before.z)<.2)throw new Error(`Movement did not resume ${JSON.stringify({before,after})}`);
  if(errors.length)throw new Error(errors.join('\n'));
  console.log('Modal/popover Pointer Lock lifecycle passed',{base,headed,before,after,final:await state()});
