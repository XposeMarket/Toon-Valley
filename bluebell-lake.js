(() => {
  ...
  const waterMat=new THREE.MeshPhongMaterial({color:0x48bce8,shininess:95,specular:0xcdf7ff,transparent:true,opacity:.96});
  ...
  // Dock starts on dry ground beside the lake-approach road and extends into water.
  ...
  const lakeFlag=new THREE.Group();lakeFlag.position.set(0,0,-2.4);dock.add(lakeFlag);const flagPole=new THREE.Mesh(new THREE.CylinderGeometry(.045,.065,5.2,6),TV.materials.dark);flagPole.position.y=2.6;lakeFlag.add(flagPole);const flag=new THREE.Mesh(TV.unitBox,new THREE.MeshBasicMaterial({color:0x59d7ff,fog:false}));flag.scale.set(1.6,.65,.06);flag.position.set(.8,4.45,0);lakeFlag.add(flag);
  TV.registerInteraction({x:82,z:-70,radius:4.2,area:'world',prompt:'Visit Bluebell Lake dock',action:()=>TV.showToast('🌊 Bluebell Lake · follow the blue beacon/flag to the dock, rowboat, shoreline fishing, reeds, and lilies.',2.8)});
  ...
  // Fishing rig: the rod, curved line, and bobber are all in boat-local space so
  // the cast stays attached to the boat while it rocks or turns.
  const rod=new THREE.Mesh(new THREE.CylinderGeometry(.025,.04,2.7,7),TV.materials.dark);rod.rotation.x=1.04;rod.position.set(.72,1.72,.72);rod.visible=false;visual.add(rod);
  const castPoints=[];for(let i=0;i<=12;i++){const t=i/12;castPoints.push(new THREE.Vector3(.72,THREE.MathUtils.lerp(2.32,.09,t)+Math.sin(t*Math.PI)*.42,THREE.MathUtils.lerp(1.85,6.1,t)))}
  const fishingLine=new THREE.Line(new THREE.BufferGeometry().setFromPoints(castPoints),new THREE.LineBasicMaterial({color:0x1f2d35,transparent:true,opacity:.82}));fishingLine.visible=false;visual.add(fishingLine);
  const bobber=new THREE.Group(),bobTop=new THREE.Mesh(new THREE.SphereGeometry(.11,8,6),TV.materials.red),bobBottom=new THREE.Mesh(new THREE.SphereGeometry(.11,8,6),TV.materials.white);bobTop.position.y=.08;bobBottom.position.y=-.06;bobber.add(bobTop,bobBottom);bobber.position.set(.72,.09,6.1);bobber.visible=false;visual.add(bobber);
  ...
  let riding=false,speed=0,lastFish=-99,clock=0,casting=false,castTimer=0;const keys=Object.create(null);
  function controls(){document.getElementById('bl-controls')?.remove();const d=document.createElement('div');d.id='bl-controls';d.innerHTML='<span>Joystick/WASD rows · steer left/right</span><button data-fish>🎣 CAST</button><button data-leave>⚓ DOCK</button>';document.body.appendChild(d);d.querySelector('[data-fish]').onclick=fish;d.querySelector('[data-leave]').onclick=leave;}
  function board(){if(riding)return;riding=true;controls();TV.state.seated=false;TV.playerVelocity.set(0,0,0);TV.state.cameraReady=false;TV.showToast('🚣 Row around Bluebell Lake. Use the joystick or WASD; cast a real line from anywhere on the water.',3);}
  function resetFishingRig(){casting=false;castTimer=0;rod.visible=false;fishingLine.visible=false;bobber.visible=false;const b=document.querySelector('#bl-controls [data-fish]');if(b)b.textContent='🎣 CAST';}
  function leave(){if(!riding)return;riding=false;speed=0;resetFishingRig();document.getElementById('bl-controls')?.remove();TV.player.position.set(82,TV.terrainHeight(82,-70),-70);TV.playerVelocity.set(0,0,0);TV.state.grounded=true;TV.state.cameraReady=false;for(const k of Object.keys(keys))delete keys[k];TV.showToast('⚓ Back on the dock.',1.8);}
  function finishCatch(){
    if(!casting)return;const catches=[['sunfish',18],['bluegill',24],['silver trout',38],['golden carp',55]],pick=catches[Math.floor(Math.random()*catches.length)];Life.addMoney(pick[1],`Bluebell Lake ${pick[0]}`);Life.emitProgress('explore',1,{activity:'boat-fishing',fish:pick[0]});TV.showToast(`🎣 Caught a ${pick[0]}! +$${pick[1]}`,2.4);resetFishingRig();
  }
  function fish(){
    if(!riding)return;if(casting){TV.showToast('The bobber is already in the water.',1.3);return}const now=performance.now()/1000;if(now-lastFish<3.5){TV.showToast('Let the water settle for a second.',1.4);return}lastFish=now;casting=true;castTimer=1.45;rod.visible=true;fishingLine.visible=true;bobber.visible=true;const b=document.querySelector('#bl-controls [data-fish]');if(b)b.textContent='🎣 WAIT…';TV.showToast('🎣 Cast out… watch the bobber for a bite.',1.5);
  }
