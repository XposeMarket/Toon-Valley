(() => {
  ...
  // Modal implementations already construct their DOM and set modalOpen before
  // releasing Pointer Lock. Do not intercept or replace the browser API here.
  // We only repair the core pause-screen response after the native unlock event.
  document.addEventListener('pointerlockchange', () => {
    if (TV.DEVICE.touch || gamePointerLocked()) return;
    if (TV.state.modalOpen || modalUIVisible() || resumeAfterModal) {
