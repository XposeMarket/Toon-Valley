(() => {
  ...
  function makeHiker(index, startIndex, direction) {
    ...
    root.add(group);
    const state = { group, leftLeg, rightLeg, index: startIndex, targetIndex: startIndex + direction, direction, pause: index ? .8 : .2, yielding: 0, distance: 0, landmarkPauses: 0, yieldEvents: 0 };
    hikers.push(state);
    return state;
  }
  ...
  function advanceHiker(hiker, dt) {
    ...
    if (playerDistance < 2.45 && hiker.yielding <= 0) {
      hiker.yielding = 1.05;
      hiker.yieldEvents += 1;
      totalYieldEvents += 1;
    }
    ...
    if (dist <= Math.max(.12, speed * dt)) {
      ...
      if (isLandmark(hiker.index)) { hiker.pause = hiker.index === 11 ? 2.4 : 1.15; hiker.landmarkPauses += 1; }
  ...
  function getState() {
    return {
      ...
      hikers: hikers.map(h => ({
        ...
        distance: h.distance, landmarkPauses: h.landmarkPauses, yieldEvents: h.yieldEvents,
